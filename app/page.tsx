  ...
  return (
    <main className="container app-shell">
      <div className="app-header">
        <div>
          <p className="brand">BetterBeFit</p>
          <p className="subtitle">Calm, consistent recomposition.</p>
        </div>

        <div className="header-actions">
          <div className="streak-pill">🔥 {streakDays} day streak</div>
          <button className="btn secondary compact-btn" onClick={exportBackup}>
            Export
          </button>
          <button className="btn secondary compact-btn" onClick={() => fileInputRef.current?.click()}>
            Import
          </button>
          <input
            ref={fileInputRef}
        ...
        <div className="dashboard-shell">
          <section className="xp-card">
            <div className="row space-between">
              <div>
                <div className="small">Weekly consistency</div>
                <strong>Level {weeklyXp.level} — {weeklyXp.title}</strong>
              </div>
              <div className="small">{weeklyXp.xp} XP earned</div>
            </div>
            <div className="progress-track">
              <div className="progress-fill" style={{ width: `${weeklyXp.progress}%` }} />
            </div>
            <div className="row space-between">
              <span className="small">{weeklyXp.xp} XP this week</span>
          ...
          <section>
            ...
            <div className="week-strip">
              {weekStatus.map((day) => (
                <div key={day.label} className={`day-chip ${day.status}`} title={day.status}>
                  {day.label}
                </div>
              ))}
            </div>
          ...
          <section className="metric-grid">
            <SignalCard title="Protein" value={formatNumber(avgProtein, "g")} hint={`${Math.round((avgProtein ?? 0) - state.settings.proteinTarget)}g vs target`} status={(avgProtein ?? 0) >= state.settings.proteinTarget ? "good" : "warn"} />
            <SignalCard title="Steps" value={formatNumber(avgSteps)} hint={`${state.settings.stepTarget.toLocaleString()} target`} status={(avgSteps ?? 0) >= state.settings.stepTarget ? "good" : "warn"} />
            <SignalCard title="Calories" value={formatNumber(avgCalories)} hint={`${state.settings.targetCalories} target`} status={avgCalories !== null && Math.abs(avgCalories - state.settings.targetCalories) <= 150 ? "good" : "warn"} />
            <SignalCard title="Workouts" value={`${workoutCompletion}%`} hint="log completion" status={workoutCompletion >= 70 ? "good" : "warn"} />
          </section>

          <section className="readiness-card">
            <div className="ring" style={{ background: `conic-gradient(#24b38a ${readinessScore * 3.6}deg, #263244 0deg)` }}>
          ...
          <section>
            ...
            <div className="signal-list">
              {todaySignals.map((item) => (
                <div className={`coach-signal ${item.type}`} key={item.text}>
            ...
            <div className="card">
              ...
              <div className="advice-list">
                {cardioRoutine.map((item) => (
                  <div className="advice-item" key={item}>{item}</div>
                ))}
              </div>
            </div>
...
function SignalCard({
  ...
  return (
    <div className={`signal-card ${status}`}>
      ...
      <div className="signal-pill">{hint}</div>
    </div>
  );
}
...
function getReadinessMessage(score: number) {
  if (score >= 85) return "You are locked in. Hold the plan and avoid unnecessary changes.";
  if (score >= 70) return "Strong week. One or two small fixes will keep recomp moving.";
  if (score >= 50) return "You have momentum, but consistency needs tightening today.";
  return "Simplify today: hit protein, walk, and log the basics.";
}
...
function getTodaySignals(input: {
  ...
  if ((input.avgProtein ?? input.foodTotals.protein) >= input.settings.proteinTarget) {
    signals.push({ type: "good", text: "Protein is locked in. This protects muscle while you recomp." });
  } else {
  ...
  if ((input.avgSteps ?? 0) >= input.settings.stepTarget) {
    signals.push({ type: "good", text: "Step target is on track. NEAT is doing its job." });
  } else {
  ...
  if (Math.abs(calories - input.settings.targetCalories) <= 150) {
    signals.push({ type: "good", text: "Calories are close to target. Avoid unnecessary changes today." });
  } else if (calories < input.settings.targetCalories - 250) {
    signals.push({ type: "warn", text: "Calories are running low. Under-eating can blunt recovery and stall recomp." });
  } else {
...
function getWeeklyXp(input: {
  ...
  weekStatus: Array<{ label: string; status: string }>;
  workoutCompletion: number;
}) {
  const loggedDays = input.weekStatus.filter((day) => day.status === "done").length;
  const xp = Math.min(1000, Math.round(input.readinessScore * 6 + loggedDays * 40 + input.workoutCompletion * 2));
